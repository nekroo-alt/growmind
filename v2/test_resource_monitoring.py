...
def test_resource_monitoring():
    ...
    try:
        ...
        op_id = telemetry.start_operation(
            operation_type="test",
            title="Resource Monitoring Test"
        )
        ...
        if "error" not in cpu_metrics:
            ...
            print(f"    CPU: {cpu_metrics['cpu_percent']}% across {cpu_metrics['cpu_count']} cores")
        else:
        ...
        if "error" not in mem_metrics:
            ...
            print(f"    Memory: {mem_metrics['percent']}% of {mem_metrics['total_mb']:.1f}MB")
        else:
        ...
        if "error" not in disk_metrics:
            ...
            print(f"    Disk: {disk_metrics['percent']}% of {disk_metrics['total_gb']:.1f}GB")
        else:
        ...
        if "error" not in io_metrics:
            ...
            print(f"    I/O: Read {io_metrics['read_bytes_mb']:.2f}MB, Write {io_metrics['write_bytes_mb']:.2f}MB")
        else:
        ...
        if "error" not in net_metrics:
            ...
            print(f"    Network: Sent {net_metrics['bytes_sent_mb']:.2f}MB, Recv {net_metrics['bytes_recv_mb']:.2f}MB")
        else:
...
def test_resource_thresholds():
    ...
    try:
        ...
        if "error" not in cpu_metrics and "error" not in mem_metrics:
            ...
            for alert in alerts:
                print(f"    - {alert['resource_type']}: {alert['severity']} - {alert['message']}")
...
def test_resource_monitoring_context_manager():
    ...
    try:
        ...
        if "error" not in summary:
            ...
            print(f"    CPU: min={summary['cpu']['min']:.1f}%, max={summary['cpu']['max']:.1f}%, avg={summary['cpu']['avg']:.1f}%")
            print(f"    Memory: min={summary['memory']['min']:.1f}%, max={summary['memory']['max']:.1f}%, avg={summary['memory']['avg']:.1f}%")
            print(f"    Disk: min={summary['disk']['min']:.1f}%, max={summary['disk']['max']:.1f}%, avg={summary['disk']['avg']:.1f}%")
...
def test_resource_trends():
    ...
    try:
        ...
        for i in range(5):
            op_id = telemetry.start_operation(
                operation_type="test",
                title=f"Trend Test Operation {i+1}"
            )
