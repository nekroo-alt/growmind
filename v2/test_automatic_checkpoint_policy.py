...
def test_checkpoint_policy_default():
    ...
    assert policy.critical_types == ['task_complete', 'operation_end', 'refactor_complete']
...
def test_auto_checkpoint_before_task():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(before_task=True)
    )
...
def test_auto_checkpoint_before_task_disabled():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(before_task=False)
    )
...
def test_auto_checkpoint_after_task():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(after_task=True)
    )
...
def test_auto_checkpoint_after_task_disabled():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(after_task=False)
    )
...
def test_auto_checkpoint_before_refactor():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(before_refactor=True)
    )
    
    checkpoint_id = manager.auto_checkpoint_before_refactor(
        operation_id="op-123"
    )
...
def test_auto_checkpoint_after_refactor():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(after_refactor=True)
    )
    
    checkpoint_id = manager.auto_checkpoint_after_refactor(
        operation_id="op-123"
    )
...
def test_auto_checkpoint_on_error():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(on_error=True)
    )
...
def test_auto_checkpoint_on_error_disabled():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(on_error=False)
    )
...
def test_auto_checkpoint_on_interrupt():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(on_interrupt=True)
    )
...
def test_auto_checkpoint_after_operation():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(after_n_operations=3)
    )
...
def test_auto_checkpoint_after_operation_disabled():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(after_n_operations=None)
    )
...
def test_auto_cleanup():
    ...
    manager = CheckpointManager(
        policy=CheckpointPolicy(max_count=3, max_age_hours=1)
    )
...
def test_cleanup_respects_critical_checkpoints():
    ...
    critical_id1 = manager.create(
        reason="Critical 1",
        snapshot_type="task_complete"
    )
    critical_id2 = manager.create(
        reason="Critical 2",
        snapshot_type="refactor_complete"
    )
