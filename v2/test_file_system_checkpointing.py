...
def test_file_restore():
    ...
    try:
        ...
        assert current_content == modified_content, "File should still be modified after dry-run"
...
def test_trackable_files():
    ...
    for filename, expected in test_cases:
        ...
        assert result == expected, f"File {filename} should be {'tracked' if expected else 'not tracked'}"
...
def test_checkpoint_cleanup():
    ...
    checkpoints = manager.list(limit=10)
    assert len(checkpoints) <= 2, f"Should have at most 2 checkpoints, got {len(checkpoints)}"
