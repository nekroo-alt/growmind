...
def test_message_templates():
    ...
    msg = format_log_message(LogMessageTemplates.OPERATION_STARTED, operation_type="implementation")
    assert msg == "implementation started", f"Expected 'implementation started', got '{msg}'"
    print(f"✓ Operation started template: {msg}")
    
    msg = format_log_message(LogMessageTemplates.OPERATION_COMPLETED, operation_type="implementation")
    assert msg == "implementation completed", f"Expected 'implementation completed', got '{msg}'"
    print(f"✓ Operation completed template: {msg}")
    
    msg = format_log_message(LogMessageTemplates.OPERATION_FAILED, 
                            operation_type="implementation", error="File not found")
    assert msg == "implementation failed: File not found", f"Expected 'implementation failed: File not found', got '{msg}'"
    print(f"✓ Operation failed template: {msg}")
    
    # Test task templates
    msg = format_log_message(LogMessageTemplates.TASK_STARTED, 
                            task_id=42, task_title="Add authentication")
    assert msg == "Task 42 started: Add authentication", f"Expected 'Task 42 started: Add authentication', got '{msg}'"
    ...
    msg = format_log_message(LogMessageTemplates.CONTEXT_COLLECTION_COMPLETED,
                            num_files=5, num_tokens=1200)
    assert msg == "Context collection completed: 5 files, 1200 tokens", f"Expected 'Context collection completed: 5 files, 1200 tokens', got '{msg}'"
    print(f"✓ Context collection template: {msg}")
    
    # Test telemetry templates
    msg = format_log_message(LogMessageTemplates.TELEMETRY_OPERATION_TRACKED,
                            operation_type="implementation", operation_id="abc-123")
    assert msg == "Telemetry tracked: implementation (id: abc-123)", f"Expected 'Telemetry tracked: implementation (id: abc-123)', got '{msg}'"
...
def test_helper_functions():
    ...
    log_operation_completed(logger, "implementation", operation_id="abc-123", task_id=42)
    print("✓ Operation completed logged")
    
    # Test task logging
    print("Testing log_task_started...")
    log_task_started(logger, task_id=42, task_title="Add authentication", operation_id="abc-123")
    ...
    log_operation_failed(logger, "implementation", "File not found", 
                        operation_id="abc-123", task_id=42)
...
def test_context_injection():
    ...
    logger.info("Another test message", extra={
        'operation_id': 'xyz-789',
