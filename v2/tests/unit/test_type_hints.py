...
def test_type_hint_extraction():
    ...
    print("-" * 70)
    simple_func = next((f for f in summary["functions"] if f["name"] == "simple_function"), None)
    if simple_func:
    ...
    print("-" * 70)
    complex_func = next((f for f in summary["functions"] if f["name"] == "complex_function"), None)
    if complex_func:
    ...
    print("-" * 70)
    no_hints_func = next((f for f in summary["functions"] if f["name"] == "no_hints"), None)
    if no_hints_func:
    ...
    if typed_class:
        ...
        init_method = next((m for m in typed_class["methods"] if m["name"] == "__init__"), None)
        if init_method:
        ...
        get_name = next((m for m in typed_class["methods"] if m["name"] == "get_name"), None)
        if get_name:
    ...
    if typed_class:
        print(f"Class: {typed_class['name']}")
        print(f"Attribute type hints: {json.dumps(typed_class['attribute_type_hints'], indent=2)}")
    ...
    print("-" * 70)
    annotated_class = next((c for c in summary["classes"] if c["name"] == "AnnotatedClass"), None)
    if annotated_class:
        print(f"Class: {annotated_class['name']}")
        print(f"Attribute type hints: {json.dumps(annotated_class['attribute_type_hints'], indent=2)}")
        ...
        assert annotated_class['attribute_type_hints']['config']['type'] == 'Dict[str, Optional[int]]'
        print("✓ __annotations__ parsed correctly")
    
    # Test 7: Union types (Python 3.9+)
    print("\n7. Testing union types:")
    print("-" * 70)
    union_func = next((f for f in summary["functions"] if f["name"] == "union_type"), None)
    if union_func:
        ...
        print(f"  Parameter 'value' type: {union_func['type_hints']['parameters'].get('value', 'N/A')}")
