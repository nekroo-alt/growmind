...
from v2.core.session_manager import (
    SessionManager,
    Session,
    SessionStatus,
    get_session_manager
...
class TestSessionLifecycle:
    ...
    def test_session_status_transitions(self, manager):
        ...
        assert manager._load_session(session.session_id).status == SessionStatus.COMPLETED
...
class TestSessionRecovery:
    ...
    def test_restore_session_on_startup(self, manager, monkeypatch):
        ...
        restored, has_changes = manager.restore_session_on_startup(
            session.session_id,
            checkpoint_manager=None  # Not checking external changes
        )
...
class TestSessionAnalytics:
    ...
    def test_get_session_metrics_with_telemetry(self, manager):
        ...
        metrics = manager.get_session_metrics(session.session_id, telemetry_manager=None)
        
        assert metrics["tasks_completed"] == 2
        assert metrics["operations_count"] == 0  # No operations tracked without telemetry
    ...
    def test_get_session_productivity_with_errors(self, manager):
        ...
        class MockTelemetryManager:
            def query_operations(self, **kwargs):
                return [
                    {"status": "completed", "start_time": datetime.now().isoformat(), "end_time": datetime.now().isoformat()},
                    {"status": "failed", "start_time": datetime.now().isoformat(), "end_time": datetime.now().isoformat()}
                ]
    ...
    def test_identify_bottlenecks_without_telemetry(self, manager):
        ...
        bottlenecks = manager.identify_bottlenecks(session.session_id, telemetry_manager=None)
    ...
    def test_identify_error_patterns_without_telemetry(self, manager):
        ...
        patterns = manager.identify_error_patterns(session.session_id, telemetry_manager=None)
