...
def test_session_schema_initialization():
    ...
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='session_operations'")
    assert cursor.fetchone() is not None, "session_operations table should exist"
    
    # Check session_checkpoints table
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='session_checkpoints'")
    assert cursor.fetchone() is not None, "session_checkpoints table should exist"
    
    # Check session_config table
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='session_config'")
    assert cursor.fetchone() is not None, "session_config table should exist"
    
    # Check indexes
    cursor.execute("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_sessions_status'")
    assert cursor.fetchone() is not None, "idx_sessions_status index should exist"
    
    cursor.execute("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_session_ops_session'")
...
def test_session_statistics():
    ...
    # Track operations
    op1 = track_session_operation(session_id, "op_001", "implementation", 1, "in_progress")
    update_session_operation_status(op1, "completed")
    
    op2 = track_session_operation(session_id, "op_002", "verification", 1, "in_progress")
    update_session_operation_status(op2, "completed")
    
    op3 = track_session_operation(session_id, "op_003", "implementation", 2, "in_progress")
