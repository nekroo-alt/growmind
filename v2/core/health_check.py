...
class HealthCheckManager:
            ...
            try:
                ...
                if integrity_result and integrity_result[0] == "ok":
                    ...
                    results.append(HealthCheckResult(
                        ...
                        details={"path": db_path, "integrity_result": integrity_result},
                        fixable=True
                    ))
            ...
            if os.path.exists(db_path):
                ...
                if readable and writable:
                    results.append(HealthCheckResult(
                        ...
                        details={"path": db_path, "readable": readable, "writable": writable}
                    ))
                else:
                    results.append(HealthCheckResult(
                        name=f"{db_name}_database_permissions",
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if not status_lines:
                    ...
                    modified_count = len([l for l in status_lines if l and not l.startswith('??')])
    ...
    def _check_cache(self) -> List[HealthCheckResult]:
        ...
        try:
            ...
            if cache_size_mb > self.CRITICAL_CACHE_SIZE_MB:
                ...
                message = f"Cache size {cache_size_mb:.2f} MB exceeds critical threshold"
    ...
    def _generate_recommendations(self, checks: List[HealthCheckResult]) -> List[str]:
        ...
        priority_order = [HealthStatus.CRITICAL, HealthStatus.ERROR, HealthStatus.WARNING]
        
        for status in priority_order:
            for check in checks:
                if check.status == status and check.recommendation:
                    recommendations.append(f"[{status.value.upper()}] {check.name}: {check.recommendation}")
    ...
    def print_report(self, report: HealthReport, verbose: bool = False):
        ...
        summary_table.add_row("[yellow]WARNING[/yellow]", str(report.summary["warning"]))
        summary_table.add_row("[red]ERROR[/red]", str(report.summary["error"]))
        summary_table.add_row("[red bold]CRITICAL[/red bold]", str(report.summary["critical"]))
        ...
        if verbose:
            for check in report.checks:
                ...
                value_str = f" ({check.value:.2f} {check.unit})" if check.value and check.unit else ""
    ...
    def _print_simple_report(self, report: HealthReport, verbose: bool = False):
        ...
        if verbose:
            ...
            for check in report.checks:
                value_str = f" ({check.value:.2f} {check.unit})" if check.value and check.unit else ""
                fixable_str = " [FIXABLE]" if check.fixable else ""
                print(f"  [{check.status.value.upper()}] {check.name}{value_str}{fixable_str}")
                ...
                try:
                    if check.name.startswith("cache_"):
                        ...
                        if os.path.exists(cache_dir):
                            ...
                            fix_results.append(f"✓ Cleared cache directory: {cache_dir}")
                            fixes_attempted += 1
                        ...
                        for db_path in self._db_paths.values():
                            if os.path.exists(db_path):
                                ...
                                fix_results.append(f"✓ Fixed permissions for: {db_path}")
                                fixes_attempted += 1
