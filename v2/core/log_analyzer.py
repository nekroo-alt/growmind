    ...
    def __init__(self, data: Dict[str, Any]):
        ...
        self.extra = {k: v for k, v in data.items() 
                     if k not in ['timestamp', 'level', 'logger', 'message', 
                                 'module', 'function', 'line', 'operation_id',
                                 'task_id', 'session_id', 'exception']}
        self.raw_data = data
...
class LogAnalyzer:
    ...
    def identify_error_patterns(self) -> List[Dict[str, Any]]:
        ...
        for error_type, error_list in error_types.items():
            ...
            patterns.append({
                ...
                'message_patterns': dict(sorted(message_patterns.items(), 
                                                key=lambda x: x[1], reverse=True)[:5]),
                'first_occurrence': min(e.timestamp for e in error_list if e.timestamp).isoformat() 
                                   if any(e.timestamp for e in error_list) else None,
                'last_occurrence': max(e.timestamp for e in error_list if e.timestamp).isoformat()
                                  if any(e.timestamp for e in error_list) else None
            })
    ...
    def export_to_csv(self, query: Optional[LogQuery] = None, output_file: str = "logs_export.csv") -> None:
        """
        ...
        fieldnames = [
            ...
            'function', 'line', 'operation_id', 'task_id', 'session_id'
        ]
    ...
    def export_to_json(self, query: Optional[LogQuery] = None, output_file: str = "logs_export.json") -> None:
        """
