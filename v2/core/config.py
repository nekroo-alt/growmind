...
class CacheConfig:
    ...
    def __post_init__(self):
        ...
        if self.max_size_mb < 1:
            raise ValueError(f"Max cache size must be at least 1MB, got {self.max_size_mb}")
...
class CheckpointConfig:
    ...
    def __post_init__(self):
        ...
        if self.max_age_hours < 1:
            raise ValueError(f"Max checkpoint age must be at least 1 hour, got {self.max_age_hours}")
        if self.max_count < 1:
            raise ValueError(f"Max checkpoint count must be at least 1, got {self.max_count}")
...
class ConfigManager:
        ...
        if self.config_file.exists():
            try:
                ...
                if file_version != self.CONFIG_VERSION:
                    logger.info(f"Config version {file_version}, current {self.CONFIG_VERSION}")
    ...
    def _migrate_config(self, from_version: str, config: Dict[str, Any]) -> Dict[str, Any]:
