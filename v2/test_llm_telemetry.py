...
def test_llm_telemetry_integration():
    ...
    for metric in metrics:
        print(f"  - {metric['metric_name']}: {metric['metric_value']} {metric.get('unit', '')}")
...
def test_llm_latency_tracking():
    ...
    # Find latency metric
    latency_metric = next((m for m in metrics if m['metric_name'] == 'latency_seconds'), None)
    ...
    assert abs(tracked_latency - actual_duration) < 0.1, \
...
def test_llm_token_counting():
    ...
    # Check token counts
    prompt_tokens = next(m['metric_value'] for m in metrics if m['metric_name'] == 'prompt_tokens')
    completion_tokens = next(m['metric_value'] for m in metrics if m['metric_name'] == 'completion_tokens')
    total_tokens = next(m['metric_value'] for m in metrics if m['metric_name'] == 'total_tokens')
    
    assert prompt_tokens > 0, "prompt_tokens should be > 0"
    assert completion_tokens > 0, "completion_tokens should be > 0"
    assert total_tokens == prompt_tokens + completion_tokens, "total_tokens should equal sum"
    ...
    assert completion_tokens == result['usage']['completion_tokens'], "Completion token mismatch"
...
def test_llm_response_size_tracking():
    ...
    # Check response size
    size_metric = next((m for m in metrics if m['metric_name'] == 'response_size_chars'), None)
    ...
    assert tracked_size == actual_size, f"Size mismatch: tracked={tracked_size}, actual={actual_size}"
