...
def test_context_engine_state_capture():
    ...
    if context_state:
        ...
        print(f"Expected state components: {list(context_data.get('expected_state', {}).keys())}")
...
def test_checkpoint_with_all_states():
    ...
    assert checkpoint_data.get('db_state') is not None, "Database state should be present"
    ...
    assert checkpoint_data.get('cache_state') is not None, "Cache state should be present"
...
def test_checkpoint_list_with_cache():
    ...
    cache_checkpoints = [c for c in checkpoints if c.get('metadata', {}).get('include_cache')]
...
def test_checkpoint_metadata_includes_cache():
    ...
    assert metadata.get('include_databases') == False, "Metadata should show databases not included"
    assert metadata.get('include_files') == False, "Metadata should show files not included"
