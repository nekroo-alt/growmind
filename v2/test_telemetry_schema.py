...
def test_telemetry_schema_creation():
    ...
    try:
        ...
        assert columns['parent_id'] in ['TEXT', 'VARCHAR', None], "operations.parent_id should be TEXT or nullable"
        assert columns['operation_type'] in ['TEXT', 'VARCHAR'], "operations.operation_type should be TEXT"
        assert columns['title'] in ['TEXT', 'VARCHAR'], "operations.title should be TEXT"
        assert columns['status'] in ['TEXT', 'VARCHAR'], "operations.status should be TEXT"
        assert columns['start_time'] in ['TEXT', 'VARCHAR', 'DATETIME'], "operations.start_time should be TEXT or DATETIME"
        assert columns['end_time'] in ['TEXT', 'VARCHAR', 'DATETIME', None], "operations.end_time should be TEXT or DATETIME or nullable"
        assert columns['metadata'] in ['TEXT', 'VARCHAR', None], "operations.metadata should be TEXT or nullable"
        assert columns['activity_id'] in ['INTEGER', 'INT', None], "operations.activity_id should be INTEGER or nullable"
        
        # Verify events table schema
        cursor.execute("PRAGMA table_info(events)")
        columns = {row[1]: row[2] for row in cursor.fetchall()}
        assert columns['id'] in ['TEXT', 'VARCHAR'], "events.id should be TEXT"
        assert columns['operation_id'] in ['TEXT', 'VARCHAR'], "events.operation_id should be TEXT"
        assert columns['event_type'] in ['TEXT', 'VARCHAR'], "events.event_type should be TEXT"
        assert columns['severity'] in ['TEXT', 'VARCHAR'], "events.severity should be TEXT"
        assert columns['timestamp'] in ['TEXT', 'VARCHAR', 'DATETIME'], "events.timestamp should be TEXT or DATETIME"
        assert columns['message'] in ['TEXT', 'VARCHAR'], "events.message should be TEXT"
        assert columns['context'] in ['TEXT', 'VARCHAR', None], "events.context should be TEXT or nullable"
        
        # Verify metrics table schema
        cursor.execute("PRAGMA table_info(metrics)")
        columns = {row[1]: row[2] for row in cursor.fetchall()}
        assert columns['id'] in ['TEXT', 'VARCHAR'], "metrics.id should be TEXT"
        assert columns['operation_id'] in ['TEXT', 'VARCHAR'], "metrics.operation_id should be TEXT"
        assert columns['metric_name'] in ['TEXT', 'VARCHAR'], "metrics.metric_name should be TEXT"
        assert columns['metric_value'] in ['REAL', 'FLOAT', 'DOUBLE'], "metrics.metric_value should be REAL"
        assert columns['unit'] in ['TEXT', 'VARCHAR', None], "metrics.unit should be TEXT or nullable"
        assert columns['timestamp'] in ['TEXT', 'VARCHAR', 'DATETIME'], "metrics.timestamp should be TEXT or DATETIME"
        
        # Verify resources table schema
        cursor.execute("PRAGMA table_info(resources)")
        columns = {row[1]: row[2] for row in cursor.fetchall()}
        assert columns['id'] in ['TEXT', 'VARCHAR'], "resources.id should be TEXT"
        assert columns['operation_id'] in ['TEXT', 'VARCHAR'], "resources.operation_id should be TEXT"
        assert columns['resource_type'] in ['TEXT', 'VARCHAR'], "resources.resource_type should be TEXT"
        assert columns['resource_name'] in ['TEXT', 'VARCHAR', None], "resources.resource_name should be TEXT or nullable"
        assert columns['value'] in ['REAL', 'FLOAT', 'DOUBLE'], "resources.value should be REAL"
        assert columns['unit'] in ['TEXT', 'VARCHAR', None], "resources.unit should be TEXT or nullable"
        assert columns['timestamp'] in ['TEXT', 'VARCHAR', 'DATETIME'], "resources.timestamp should be TEXT or DATETIME"
        
        # Verify migrations table schema
        cursor.execute("PRAGMA table_info(migrations)")
        columns = {row[1]: row[2] for row in cursor.fetchall()}
        assert columns['version'] in ['INTEGER', 'INT'], "migrations.version should be INTEGER"
        assert columns['applied_at'] in ['TEXT', 'VARCHAR', 'DATETIME'], "migrations.applied_at should be TEXT or DATETIME"
        assert columns['description'] in ['TEXT', 'VARCHAR'], "migrations.description should be TEXT"
...
def test_resources():
    ...
    try:
        ...
        mem_resource = next((r for r in resources if r['resource_type'] == 'memory'), None)
