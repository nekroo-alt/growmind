...
class TaskImpactAnalyzer:
    ...
    def analyze_task_impact(
        ...
        result = {
            ...
            "analysis_metadata": {
                ...
                "high_impact_files": len([f for f in scored_files if f["impact_score"] >= 0.7])
            }
        }
    ...
    def _extract_entities_from_task(
        ...
        result = self.llm.call(system_prompt, user_prompt, temperature=0.3, max_tokens=1024)
    ...
    def _find_files_with_entities(
        self, 
        entities: Dict, 
        project_modules: Optional[List[str]]
    ) -> Dict:
        """
                ...
                for method in cls.get("methods", []):
                    for func_name in entities.get("functions", []):
                        if func_name == method["name"]:
                            matches["functions_matched"].append(f"{cls['name']}.{method['name']}")
    ...
    def _calculate_impact_scores(
        ...
        for file_path, matches in file_matches.items():
            ...
            module_score = (len(matches["modules_matched"]) / total_modules) * module_weight
            class_score = (len(matches["classes_matched"]) / total_classes) * class_weight
            function_score = (len(matches["functions_matched"]) / total_functions) * function_weight
            keyword_score = (len(matches["keywords_matched"]) / total_keywords) * keyword_weight
