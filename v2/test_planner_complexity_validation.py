...
'''
    ...
    print(f"  simple_function complexity: {simple_complexity['complexity']} ({simple_complexity['level']})")
    assert simple_complexity['complexity'] <= 5, "Simple function should have low complexity"
    
    # Analyze complex function
    complex_complexity = estimator.calculate_function_complexity("complex_function")
    print(f"  complex_function complexity: {complex_complexity['complexity']} ({complex_complexity['level']})")
    assert complex_complexity['complexity'] > 5, "Complex function should have higher complexity"
    
    # Test 2: Verify line limit prediction
    print("\nTest 2: Line limit prediction")
    
    # Simple task should not exceed limit
    simple_prediction = estimator.will_exceed_line_limit(["simple_function"], threshold=30)
    print(f"  Simple task will exceed 30 lines: {simple_prediction['will_exceed']}")
    print(f"  Reasoning: {simple_prediction['reasoning']}")
    assert not simple_prediction['will_exceed'], "Simple function should not exceed 30-line limit"
    ...
    try:
        ...
        assert full_path in planner.semantic_mappers, "Semantic mapper should be created"
        
        mapper = planner.semantic_mappers[full_path]
        assert hasattr(mapper, 'complexity_estimator'), "Mapper should have complexity_estimator"
        assert isinstance(mapper.complexity_estimator, ComplexityEstimator), "Should be ComplexityEstimator instance"
        ...
        assert 'estimated_lines' in validation, "Validation should include estimated_lines"
        assert 'complexity_score' in validation, "Validation should include complexity_score"
        assert 'needs_breakdown' in validation, "Validation should include needs_breakdown"
        ...
        print(f"  Context size: {context_size_chars} chars ({context_size_lines} lines)")
