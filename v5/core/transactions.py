...
class Transaction:
    ...
    def __init__(
        ...
        self._metadata = TransactionMetadata(
            transaction_id=transaction_id, parent_id=parent_id, start_time=0.0
        )
    ...
    def _commit_transaction(self) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Committing transaction: {self.id} (steps: {len(self._metadata.steps_completed)})"
            )
            ...
            if op_id:
                ...
                if op:
                    ...
                    op.record_metric(
                        "steps_completed", len(self._metadata.steps_completed)
                    )
    ...
    def _rollback_transaction(self, reason: str) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Rolling back transaction: {self.id} (checkpoint: {self._metadata.checkpoint_id}, reason: {reason})"
            )
            ...
            if self._metadata.checkpoint_id:
                try:
                    ...
                    logger.info(
                        f"Restored from checkpoint successfully: {self._metadata.checkpoint_id}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to restore from checkpoint: {self._metadata.checkpoint_id}, transaction: {self.id}, error: {e}"
                    )
                    raise
            else:
            ...
            logger.info(
                f"Transaction rolled back successfully: {self.id}, reason: {reason}"
            )
    ...
    def record_step(self, step_name: str, success: bool) -> None:
        ...
        with self._lock:
            if success:
                ...
                logger.debug(
                    f"Transaction step completed: {self.id}, step: {step_name}"
                )
            else:
...
class TransactionManager:
    ...
    def __init__(
        self, checkpoint_manager: CheckpointManager, telemetry_manager: TelemetryManager
    ):
        self.checkpoint_manager = checkpoint_manager
        self.telemetry_manager = telemetry_manager
    ...
    @contextmanager
    def start(self, reason: str = "transaction", transaction_id: Optional[str] = None):
        """
    ...
    def get_transaction_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            stats = {
                ...
                "by_state": {
                    state.value: sum(
                        1 for tx in self._transactions.values() if tx.state == state
                    )
...
def create_transaction_manager(
    checkpoint_manager: CheckpointManager, telemetry_manager: TelemetryManager
) -> TransactionManager:
    """
    ...
    return TransactionManager(
        checkpoint_manager=checkpoint_manager, telemetry_manager=telemetry_manager
    )
...
def with_transaction(reason: str = "transaction", transaction_id: Optional[str] = None):
    """
