...
from v5.data.db_manager import log_task, log_activity, fcid_mapping, task_exists
...
from v5.llm_base.provider import LLMProvider
from v5.data.telemetry_manager import get_telemetry_manager
from v5.core.telemetry import telemetry
from v5.core.logging_config import get_module_logger, log_error_with_context
# V4: Adaptive reasoning components
from v5.data.context_hierarchy import get_context_hierarchy
from v5.data.decision_history import get_decision_history_manager
from v5.logic.reasoning_engine import get_reasoning_engine
from v5.logic.context_expander import get_context_expander
from v5.logic.progress_tracker import get_progress_tracker
from v5.logic.trap_detector import get_trap_detector
from v5.logic.trap_recovery import get_trap_recovery
# V4: Meta-cognition components
from v5.logic.pattern_recognizer import get_pattern_recognizer
from v5.logic.self_reflection import get_self_reflection
from v5.logic.lesson_learner import get_lesson_learner
from v5.logic.adaptive_heuristics import get_adaptive_heuristics
...
class Planner:
    ...
    def breakdown_requirements(
        ...
        with self.telemetry_manager.track_operation(
            ...
            logger.debug(
                f"Building semantic mappers for {len(impact_analysis['affected_files'])} files"
            )
            ...
            try:
                ...
                log_error_with_context(
                    logger, e, task_title=task_title, operation="planning"
                )
                log_activity(
            ...
            avg_complexity = (
                sum(t.get("complexity_score", 0) for t in context_aware_subtasks)
                / len(context_aware_subtasks)
                if context_aware_subtasks
                else 0
            )
            ...
            op.record_metric(
                "affected_files_count", len(impact_analysis["affected_files"])
            )
    ...
    def _build_semantic_mappers(self, affected_files: List[Dict]):
        ...
        """
        logger.debug(
            f"Building semantic mappers for {min(len(affected_files), 15)} files"
        )
    ...
    def _build_enhanced_user_prompt(
        ...
        prompt += (
            f"High-impact files identified: {len(impact_analysis['affected_files'])}\n"
        )
        prompt += (
            f"Classes likely affected: {len(code_structure_info['target_classes'])}\n"
        )
        ...
        prompt += (
            "2. Each task should target a specific class or function identified above\n"
        )
    ...
    def _validate_and_estimate_subtasks(
        self, subtasks_data: List[Dict], impact_analysis: Dict
    ) -> List[Dict]:
        """
    ...
    def _validate_task_complexity(self, task: Dict, impact_analysis: Dict) -> Dict:
        """
        ...
        if target_entities and self.semantic_mappers:
            ...
            if semantic_mapper and hasattr(semantic_mapper, "complexity_estimator"):
                ...
                limit_check = estimator.will_exceed_line_limit(
                    target_entities, threshold=30
                )
    ...
    def _enhance_acceptance_criteria(
        self, subtasks: List[Dict], impact_analysis: Dict, code_structure_info: Dict
    ) -> List[Dict]:
        """
        ...
        for task in subtasks:
            ...
            context_additions.extend(
                self._generate_context_integration_checks(
            ...
            context_additions.extend(
                self._generate_dependency_contract_checks(task, impact_analysis)
            )

            # 3. Downstream consumer protection
            context_additions.extend(
                self._generate_downstream_consumer_checks(task, impact_analysis)
            )

            # 4. Side effect testing
            context_additions.extend(
                self._generate_side_effect_checks(task, impact_analysis)
            )

            # 5. Integration test requirements
            context_additions.extend(
                self._generate_integration_test_requirements(task, impact_analysis)
            )

            # 6. Mutation testing requirements
            context_additions.extend(self._generate_mutation_test_requirements(task))

            # 7. Public API breaking change checks
            context_additions.extend(
                self._generate_api_change_checks(task, impact_analysis)
            )
            ...
            if context_additions:
                ...
                enhanced_criteria += "\n".join(
                    f"- {check}" for check in context_additions
                )
    ...
    def _generate_context_integration_checks(
        self, task: Dict, impact_analysis: Dict, code_structure_info: Dict
    ) -> List[str]:
        """
    ...
    def _generate_dependency_contract_checks(
        self, task: Dict, impact_analysis: Dict
    ) -> List[str]:
        """
    ...
    def _generate_downstream_consumer_checks(
        ...
        target_class = task.get("target_class")
        if target_class:
            checks.append(f"Public API of {target_class} remains backward compatible")
    ...
    def _generate_integration_test_requirements(
        ...
        if len(affected_files) > 1:
            checks.append(f"Integration tests verify behavior across affected files")
    ...
    def _generate_mutation_test_requirements(self, task: Dict) -> List[str]:
        """
        ...
        checks.append("Tests include mutation testing to verify test quality")
        checks.append("Mutations in critical code paths cause test failures")
    ...
    def _generate_api_change_checks(
        ...
        if target_class or target_function:
            checks.append("No breaking changes to public interfaces")
            checks.append("Backward compatibility is maintained for existing consumers")
