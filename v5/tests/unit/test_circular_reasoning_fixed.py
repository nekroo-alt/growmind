"""
Unit tests for circular reasoning detection (Task 4.4) - FIXED VERSION
...
class TestCircularReasoningContradictoryDecisions(unittest.TestCase):
    ...
    def test_contradiction_severity_critical(self):
        ...
        # Need at least 3 decisions to trigger detection
        decision_history = []
        for i in range(5):
            decision_history.append({
                "decision_id": str(i),
...
class TestCircularReasoningIntegration(unittest.TestCase):
    ...
    def test_detect_all_circular_reasoning(self):
        ...
        decision_history = [
            # Create parent_id cycle (detected by detect_infinite_recursion)
        ...
        # Check that different types are detected (infinite_recursion instead of decision_cycle)
        detected_types = set()
        for d in detections:
            type_key = d.evidence.get("circular_reasoning_type") or d.evidence.get("loop_type")
            detected_types.add(type_key)
        
        # Should detect infinite_recursion (parent_id cycle) and contradictory_decisions
        self.assertIn("infinite_recursion", detected_types)
