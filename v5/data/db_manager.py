...
def init_db():
    ...
    with sqlite3.connect(ACTIVITY_DB_PATH) as conn:
        ...
        try:
            cursor.execute(
                "ALTER TABLE activities ADD COLUMN completion_tokens INTEGER"
            )
        except sqlite3.OperationalError:
            pass  # Already exists
    ...
    with sqlite3.connect(SNAPSHOTS_DB_PATH) as conn:
        ...
        try:
            cursor.execute(
                "ALTER TABLE snapshot_db_state ADD COLUMN backup_status TEXT"
            )
        except sqlite3.OperationalError:
            pass  # Already exists
        ...
        try:
            cursor.execute(
                "ALTER TABLE snapshot_file_state ADD COLUMN backup_path TEXT"
            )
        except sqlite3.OperationalError:
            pass  # Already exists
        ...
        try:
            cursor.execute(
                "ALTER TABLE snapshot_cache_state ADD COLUMN cache_data TEXT"
            )
        except sqlite3.OperationalError:
            pass  # Already exists
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_snapshot_db ON snapshot_db_state(snapshot_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_snapshot_file ON snapshot_file_state(snapshot_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_snapshot_git ON snapshot_git_state(snapshot_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_snapshot_cache ON snapshot_cache_state(snapshot_id)"
        )
    ...
    with sqlite3.connect(SESSIONS_DB_PATH) as conn:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_session_ops_session ON session_operations(session_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_session_ops_type ON session_operations(operation_type)"
        )
        cursor.execute(
...
def validate_no_circular_dependencies(task_id, depends_on, conn=None):
    ...
    for dep_id in depends_on:
        if has_dependency_chain_to(dep_id, task_id, cursor):
            ...
            return (
                False,
                f"Circular dependency detected: task {task_id} depends on {dep_id}, but {dep_id} has a dependency chain back to {task_id}",
            )
...
def update_task_dependencies(task_id, depends_on):
    ...
    cursor.execute(
        "UPDATE tasks SET depends_on = ? WHERE id = ?", (depends_on_json, task_id)
    )
    conn.commit()
    conn.close()
...
def create_session(
    session_id=None, user=None, host=None, environment=None, metadata=None
):
    """
...
def track_session_operation(
    session_id, operation_id, operation_type, task_id=None, status="in_progress"
):
    """
