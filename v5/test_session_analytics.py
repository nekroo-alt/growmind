...
from v5.core import SessionManager, SessionStatus
...
class TestSessionMetrics:
    ...
    def test_get_session_metrics_basic(self, session_manager):
        ...
        session = session_manager.start_session(
            config={"llm_model": "gpt-4"}, metadata={"user": "test_user"}
        )
    ...
    def test_get_session_metrics_with_telemetry(
        self, session_manager, telemetry_manager
    ):
        ...
        metrics = session_manager.get_session_metrics(
            session.session_id, telemetry_manager
        )
...
class TestSessionTimeline:
    ...
    def test_generate_session_timeline_with_telemetry(
        self, session_manager, telemetry_manager
    ):
        ...
        timeline = session_manager.generate_session_timeline(
            session.session_id, telemetry_manager
        )
        ...
        operation_events = [
            e for e in timeline["events"] if e["event_type"] == "operation_start"
        ]
...
class TestBottleneckIdentification:
    ...
    def test_identify_bottlenecks_slow_operations(
        self, session_manager, telemetry_manager
    ):
        ...
        bottlenecks = session_manager.identify_bottlenecks(
            session.session_id, telemetry_manager
        )
    ...
    def test_identify_bottlenecks_failed_operations(
        self, session_manager, telemetry_manager
    ):
        ...
        telemetry_manager.record_event(
            failed_op_id, "error", "error", "Operation failed"
        )
...
class TestErrorPatterns:
    ...
    def test_identify_error_patterns(self, session_manager, telemetry_manager):
        ...
        patterns = session_manager.identify_error_patterns(
            session.session_id, telemetry_manager
        )
        ...
        assert "File not found" in [
            e["message"] for e in patterns["recurring_error_messages"]
        ]
...
class TestSessionComparison:
    ...
    def test_compare_sessions(self, session_manager):
        ...
        comparison = session_manager.compare_sessions(
            [session1.session_id, session2.session_id, session3.session_id]
        )
    ...
    def test_compare_sessions_with_telemetry(self, session_manager, telemetry_manager):
        ...
        comparison = session_manager.compare_sessions(
            [session1.session_id, session2.session_id], telemetry_manager
        )
...
class TestReportGeneration:
    ...
    def test_generate_report_without_timeline(self, session_manager):
        ...
        report = session_manager.generate_report(
            session.session_id, include_timeline=False
        )
...
class TestReportExport:
    ...
    def test_export_report_unsupported_format(self, session_manager):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            success = session_manager.export_report(
                session.session_id, export_path, format="txt"
            )
