...
class Telemetry:
    ...
    def error(
        self, message: str, exc_info=False, context: Optional[Dict[str, Any]] = None
    ):
    ...
    def log_task_failure(self, task_title: str, reason: str):
        ...
        if self.current_operation_id:
            ...
            self.telemetry_manager.end_operation(
                self.current_operation_id, "failed", {"failure_reason": reason}
            )
            self.current_operation_id = None
    ...
    def track_step(self, step_name: str, context: Optional[Dict[str, Any]] = None):
        ...
        if self.current_operation_id:
            self.telemetry_manager.record_event(
                self.current_operation_id, "step", "info", f"Step: {step_name}", context
            )
    ...
    def log_llm_usage(
        ...
        if self.current_operation_id:
            self.telemetry_manager.record_metric(
                self.current_operation_id, "tokens_used", tokens, "tokens"
            )
            self.telemetry_manager.record_metric(
                self.current_operation_id, "prompt_tokens", p_tokens, "tokens"
            )
            self.telemetry_manager.record_metric(
                self.current_operation_id, "completion_tokens", c_tokens, "tokens"
            )
            self.telemetry_manager.record_metric(
                self.current_operation_id, "cost", cost, "USD"
            )
    ...
    def task_context(
        ...
        operation_type: str = "task_execution",
    ):
        """
