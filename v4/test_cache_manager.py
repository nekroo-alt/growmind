...
from v3.data.cache_manager import CacheManager, get_cache_manager
...
class TestCacheManager:
    ...
    def test_lru_eviction(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                manager.get(str(Path(tmpdir) / "test2.py"), "semantic_map") is not None
            )
            assert (
                manager.get(str(Path(tmpdir) / "test3.py"), "semantic_map") is not None
            )
            assert (
                manager.get(str(Path(tmpdir) / "test4.py"), "semantic_map") is not None
            )
