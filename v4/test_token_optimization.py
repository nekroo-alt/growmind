...
from v3.logic.context_pruner import ContextPruner, PrunedContext
...
"""
    ...
    )  # More reasonable budget for testing
...
def test_well_understood_detection():
    ...
    assert pruner._is_well_understood_function(
        small_func
    ), "Small getter should be well-understood"
    assert not pruner._is_well_understood_function(
        large_func
    ), "Large function should not be well-understood"
    ...
    assert pruner._is_well_understood_class(
        small_class
    ), "Small class should be well-understood"
    assert not pruner._is_well_understood_class(
        large_class
    ), "Large class should not be well-understood"
    ...
    assert not pruner._is_trivial_method(
        complex_method
    ), "Complex method should not be trivial"
