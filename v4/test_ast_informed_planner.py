...
def test_planner_initialization():
    ...
    planner = Planner()
    assert hasattr(
        planner, "task_impact_analyzer"
    ), "Planner should have task_impact_analyzer"
    assert hasattr(
        planner, "semantic_mappers"
    ), "Planner should have semantic_mappers cache"
...
def test_build_semantic_mappers():
    ...
    print(
        f"✓ Build semantic mappers test passed (created {len(planner.semantic_mappers)} mappers)"
    )
...
def test_get_relevant_files():
    ...
    assert (
        len(relevant_files) == 2
    ), f"Should include 2 files, got {len(relevant_files)}"
    ...
    assert (
        "low_impact.py" not in relevant_files
    ), "Should not include low confidence file"
...
def test_validate_and_estimate_subtasks():
    ...
    subtasks_data = [
        ...
        {"title": "Task without estimate", "acceptance_criteria": "Should get default"},
    ]
...
def test_create_task_key():
    ...
    impact_analysis = {"target_classes": ["MyClass"], "target_functions": []}
    ...
    assert key == (
        ...
        "MyClass",
        None,
    ), f"Expected ('mymodule.py', 'MyClass', None), got {key}"
...
def test_prepare_code_structure_info():
    ...
    affected_files = [{"file_path": "v1/logic/planner.py", "impact_score": 0.9}]
    planner._build_semantic_mappers(affected_files)
