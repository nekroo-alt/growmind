import os
import sqlite3
from v1.data.db_manager import log_activity, init_db, ACTIVITY_DB_PATH
...
def test_log_activity_with_telemetry():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM activities WHERE summary = ? ORDER BY timestamp DESC LIMIT 1", (summary,))
    row = cursor.fetchone()
    conn.close()
