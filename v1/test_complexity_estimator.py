...
def test_function_complexity():
    ...
    print(f"   Level: {complexity['level']}")
    print(f"   Decision points: {len(complexity['decision_points'])}")
    assert complexity['complexity'] >= 2, "Moderate function should have complexity >= 2"
...
def test_task_complexity_estimation():
    ...
    assert estimate['estimated_effort'] in ["easy", "trivial"], "Simple task should be easy"
    ...
    assert estimate['estimated_effort'] in ["hard", "very_hard"], "Should be hard or very hard"
...
def test_line_limit_prediction():
    ...
    assert prediction['suggested_action'] == "break_down", "Should suggest breaking down"
    assert prediction['confidence'] in ["medium", "high"], "Should have medium or high confidence"
...
def test_refactoring_suggestions():
    ...
    assert len(suggestions) > 0, "Should find at least one suggestion for this threshold"
