...
class Telemetry:
    ...
    def __init__(self):
        ...
        self.log_file = os.path.join(self.log_dir, f"l4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    def log_task_success(self, task_title: str):
        self.info(f"✅ Task Completed Successfully: [bold green]{task_title}[/bold green]")

    def log_task_failure(self, task_title: str, reason: str):
        self.error(f"❌ Task Failed: [bold red]{task_title}[/bold red] - Reason: {reason}")
    ...
    def update_dashboard(self, task: str = None, tokens: int = None, cost: float = None, tasks_completed: int = None):
        """
        Updates the dashboard with new data.
        """
        if task: self.current_task = task
        if tokens is not None: self.stats["tokens"] = tokens
        if cost is not None: self.stats["cost"] = cost
        if tasks_completed is not None: self.stats["tasks_completed"] = tasks_completed
        ...
        self.layout["header"].update(
            Panel(
                Text("L4 Self-Evolving Platform v1.0", justify="center", style="bold magenta"),
        ...
        stats_table.add_row("Tokens Used", f"[bold cyan]{self.stats['tokens']}[/bold cyan]")
        stats_table.add_row("Est. Cost", f"[bold green]${self.stats['cost']:.4f}[/bold green]")
        stats_table.add_row("Tasks Done", f"[bold yellow]{self.stats['tasks_completed']}[/bold yellow]")
        
        self.layout["right"].update(
            Panel(stats_table, title="[bold yellow]Telemetry[/bold yellow]", border_style="yellow")
        )
