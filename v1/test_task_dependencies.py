...
def test_dependency_satisfaction():
    ...
    task = get_pending_task()
    assert task is not None and task["title"] == "Task B", "Task B should be available after Task A completes"
    ...
    task = get_pending_task()
    assert task is not None and task["title"] == "Task C", "Task C should be available after Task A and B complete"
...
def test_circular_dependency_detection():
    ...
    assert "Circular dependency" in msg, f"Expected circular dependency message, got: {msg}"
...
def test_dependency_graph():
    ...
    assert graph[3]['depends_on'] == [1, 2], f"Task 3 dependencies should be [1, 2], got {graph[3]['depends_on']}"
...
def test_dependency_chain():
    ...
    conn = sqlite3.connect(TASK_DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT id, title FROM tasks WHERE title IN ('Task D', 'Task E', 'Task F') ORDER BY id")
    ...
    assert task is not None and task["title"] == "Task D", f"Only Task D should be available, got {task['title'] if task else None}"
    print("✓ Dependency chain prevents downstream tasks from being available")
    
    # Complete the chain
    conn = sqlite3.connect(TASK_DB_PATH)
    conn.execute("UPDATE tasks SET status = 'completed' WHERE id = ?", (task_d_id,))  # Task D
    conn.execute("UPDATE tasks SET status = 'completed' WHERE id = ?", (task_e_id,))  # Task E
    conn.commit()
    conn.close()
    
    # Now Task F should be available
    task = get_pending_task()
    assert task is not None and task["title"] == "Task F", "Task F should be available after chain completes"
