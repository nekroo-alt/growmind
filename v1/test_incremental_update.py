...
""")
        ...
        update_stats = engine.update_context_incrementally(
            modified_files=None,  # Should use git diff
            task_title="Git test task"
        )
...
""")
        ...
        assert update_stats["dependency_chains_updated"] >= 0, \
...
""")
        ...
        # Trigger incremental update
        update_stats = engine.update_context_incrementally(
            modified_files=["ast_test.py"],
            task_title="AST test task"
        )
        
        # Verify AST cache was invalidated
        assert update_stats["ast_cache_invalidated"] >= 1, \
...
""")
        ...
        assert update_stats["files_analyzed"] == 2, \
            "Should analyze 2 modified files"
...
def test_no_changes_scenario():
    ...
    try:
        ...
        assert update_stats["files_analyzed"] == 0, \
            "Should analyze 0 files"
        assert update_stats["cache_entries_updated"] == 0, \
...
def test_internal_module_detection():
    ...
    assert engine._is_internal_module("v1.logic.context_engine") == True, \
        "Should detect v1 modules as internal"
    assert engine._is_internal_module("myproject.module") == True, \
        "Should detect custom modules as internal"
    
    # Test external modules (stdlib modules)
    assert engine._is_internal_module("os") == False, \
        "Should detect os as external"
    assert engine._is_internal_module("typing") == False, \
        "Should detect typing as external"
    assert engine._is_internal_module("json") == False, \
        "Should detect json as external"
