...
def test_data_flow():
    ...
    for cls in summary["classes"]:
        for method in cls["methods"]:
            assert "data_flow" in method, f"Missing data_flow in method {method['name']}"
    ...
    assert "attribute_assigns" in calc_stats_flow, "Missing 'attribute_assigns' in data flow"
    ...
    for mutation in mutations:
        ...
        for change in mutation['state_changes']:
            print(f"         * {change['object']}.{change['attribute']} (line {change['line_number']})")
    ...
    assert len(self_attrs) >= 3, f"Expected at least 3 self attributes, found {len(self_attrs)}"
    ...
    print(f"   ✓ Detected {len(process_data_flow['param_passing'])} parameter passing events:")
    for param_pass in process_data_flow['param_passing']:
        if param_pass['is_positional']:
            print(f"     - Variable '{param_pass['variable']}' passed positionally (line {param_pass['line_number']})")
        else:
            print(f"     - Variable '{param_pass['variable']}' passed as '{param_pass['parameter_name']}' (line {param_pass['line_number']})")
    ...
    print(f"   ✓ add_item attribute reads: {len(add_item_flow['attribute_reads'])} found")
