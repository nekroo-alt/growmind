...
class SemanticMapper:
    ...
    def _is_builtin(self, name):
        ...
        builtins = {
            ...
            "print", "len", "range", "getattr", "setattr", "isinstance",
            "hasattr", "type", "bool", "float", "complex",
            "abs", "all", "any", "bin", "chr", "dir", "divmod",
            "enumerate", "eval", "exec", "filter", "format", "hex",
            "id", "input", "isinstance", "issubclass", "iter",
            "locals", "map", "max", "min", "next", "oct", "open",
            "ord", "pow", "repr", "reversed", "round", "sorted",
            "sum", "vars", "zip",
    ...
    def get_state_mutations(self):
        ...
        # Check top-level functions
        for func in summary["functions"]:
            state_changes = [a for a in func["data_flow"]["attribute_assigns"] 
        ...
        for cls in summary["classes"]:
            for method in cls["methods"]:
                state_changes = [a for a in method["data_flow"]["attribute_assigns"] 
                               if a["is_self_attribute"]]
                if state_changes:
                    mutations.append({
                        "class": cls["name"],
