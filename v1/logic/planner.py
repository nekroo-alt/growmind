...
class Planner:
    ...
    def breakdown_requirements(self, product_content, technical_content, task_to_break=None):
        """
        ...
        query = task_to_break['title'] if task_to_break else "Initial requirement analysis and task breakdown"
        pruned_context = self.context_engine.get_pruned_context(query, v1_files)
        ...
        try:
            ...
            subtasks = [(t['title'], t['acceptance_criteria'], t.get('module')) for t in subtasks_data]
        except:
            # Fallback to simulated data if LLM fails or returns bad format (MVP safety)
            if task_to_break:
                subtasks = self._simulate_llm_breakdown(task_to_break['title'], pruned_context)
            else:
                subtasks = [
                    ("Initialize Core Orchestrator", "Core loop exists and starts", "core"),
                    ("Implement TDD Implementor", "Red-Green-Refactor loop works", "logic"),
                    ("Setup Acceptance Verifier", "Mutation testing passes", "logic")
                ]
