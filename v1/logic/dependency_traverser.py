...
class DependencyTraverser:
    ...
    def get_transitive_impact(
        ...
        max_reach = max(
            max(chain["upstream_depth_levels"].keys()) if chain["upstream_depth_levels"] else 0,
            max(chain["downstream_depth_levels"].keys()) if chain["downstream_depth_levels"] else 0
        )
