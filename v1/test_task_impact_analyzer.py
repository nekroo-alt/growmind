...
def test_impact_scoring():
    ...
    if len(file_matches) > 0:
        ...
        for file_info in scored_files:
            assert 0 <= file_info['impact_score'] <= 1, "Impact score must be between 0 and 1"
        
        # Verify sorted order (descending)
        scores = [f['impact_score'] for f in scored_files]
        assert scores == sorted(scores, reverse=True), "Files should be sorted by score descending"
